...
export function Header({ onToggleSidebar, showBurger = true }: HeaderProps) {
  ...
  return (
    <>
      ...
      <header
        className="sticky top-0 z-40 h-16 flex items-center justify-between px-4 md:px-6 bg-background/85 backdrop-blur-md border-b border-border"
        style={{ height: 'var(--header-height)' }}
      >
        {/* Left: burger + logo */}
        <div className="flex items-center gap-3">
          {showBurger && isAuthed && (
            <button onClick={onToggleSidebar} className="lg:hidden p-2 rounded-full hover:bg-secondary transition-colors" aria-label="Меню">
