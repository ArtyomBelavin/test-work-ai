    ...
    <>
      {!collapsed && label && (
        <div className="px-4 pt-5 pb-2">
          <span className="font-mono text-[11px] font-medium uppercase tracking-[0.14em] text-[hsl(var(--muted-foreground))]">{label}</span>
        </div>
      )}
    ...
    <>
      ...
      >
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            ...
            {!collapsed && (
              <span className="text-[20px] font-semibold tracking-tight" style={{ color: 'var(--c-fg)' }}>
                ...
                <span className="font-mono text-[13px] font-normal ml-0.5" style={{ color: 'var(--c-fg-mute)' }}>
                  .ai
                </span>
              </span>
            )}
        ...
        <div className="flex-1 overflow-y-auto space-y-0.5 scrollbar-thin">
          ...
          {!collapsed && (
            <>
              <div className="px-4 pt-5 pb-2 flex items-center justify-between">
                ...
                >
                  <ChevronDown className={cn('h-3 w-3 transition-transform duration-200', !recentOpen && '-rotate-90')} />
                  Недавние
                </button>
                <Link
                  to="/history"
                    ...
                    {recentChats.map((chat) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="flex-1 truncate text-[13px] text-foreground/90 group-hover:text-foreground">{chat.title}</span>
                          <span className="text-[10px] font-mono text-muted-foreground shrink-0">{formatTime(chat.time)}</span>
                        </Link>
                      )
                    })}
                  </div>
          ...
          {!collapsed && (
            <div
              className="mx-3 mt-5 mb-3 p-3 rounded-[12px]"
              style={{ background: 'rgba(232,84,32,0.06)', border: '1px solid rgba(232,84,32,0.15)' }}
            >
              ...
              <p className="text-[11px] text-muted-foreground leading-snug mb-2.5">Получите 100 кредитов за каждого приглашённого друга</p>
              <button
