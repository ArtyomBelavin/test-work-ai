...
const klingFunctions = [
  ...
  'Keyframes',
  'Elements',
  'Kling 01 Image to Video',
  'Kling 01 Video to Video',
  'Kling 01 Text to Video',
  'Kling Motion',
]
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
                ...
                {models.map((m) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedModel.id === m.id && 'bg-muted',
                    )}
                  >
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-popover border border-border rounded-xl shadow-lg z-10 p-1">
                  {selectedModel.subModels.map((s, i) => (
                    <button
                      ...
                      className={cn(
                        'w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-muted/60 transition-colors text-left',
                        selectedSubIndex === i && 'bg-muted',
                      )}
                    >
                      <span className="flex-1 truncate">{s.name}</span>
                      {s.isNew && (
                        <span className="text-[10px] bg-destructive text-destructive-foreground px-1.5 py-0.5 rounded-full font-semibold">NEW</span>
                      )}
                  ...
                  {klingFunctions.map((f) => (
                    <button
                      ...
                      className={cn(
                        ...
                        klingFunc === f && 'bg-muted text-primary font-medium',
                      )}
                    >
                      {f}
                    </button>
        ...
        {isVideo && (
          <div>
            ...
            {sceneOpen && <div className="text-sm text-muted-foreground text-center py-6 border border-dashed border-border rounded-xl">Скоро</div>}
          </div>
        )}
      </div>
