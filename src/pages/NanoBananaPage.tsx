...
const features: { Icon: LucideIcon; title: string; desc: string }[] = [
  {
    Icon: Target,
    title: 'Фотореализм',
    desc: 'Генерация изображений с фотографической детализацией. Кожа, текстуры, освещение — неотличимо от реальных фото.',
  },
  {
    Icon: Lock,
    title: 'Неизменные сюжеты',
    desc: 'Контроль консистентности персонажей и сцен. Один и тот же человек в разных ракурсах и ситуациях.',
  },
...
const faqItems = [
  {
    ...
    a: 'Nano Banana 2 — это нейросеть для генерации фотореалистичных изображений премиум-класса. Отличается от других моделей неизменной консистентностью персонажей и высочайшей детализацией.',
  },
  {
    q: 'Сколько стоит генерация?',
    a: 'Одна генерация Nano Banana 2 стоит 300 кредитов. Также доступны Nano Banana Pro (150 cr) и Nano Banana (80 cr) с более доступной ценой.',
  },
  { q: 'Можно ли использовать без VPN?', a: 'Да. На ERA2 Nano Banana работает напрямую из России без VPN и ограничений.' },
  {
    q: 'Чем отличается от MidJourney?',
    a: 'Nano Banana 2 специализируется на фотореализме и консистентности персонажей. MidJourney лучше для арт-стилистики и иллюстраций. Оба доступны на ERA2.',
  },
...
const NanoBananaPage = () => {
  ...
  useEffect(() => {
    ...
    if (meta)
      meta.setAttribute(
        'content',
        'Генерация фотореалистичных изображений с Nano Banana 2. Без VPN, с оплатой в рублях. Разрешение до 4K, неизменные персонажи.',
      )
  }, [])
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      <section className="pt-28 pb-16 px-4" style={{ background: 'linear-gradient(180deg, #1a0533 0%, transparent 60%)' }}>
        <div className="max-w-[800px] mx-auto text-center">
          <div className="mx-auto">
            <ModelGlyph name="Nano Banana" size={64} />
          </div>
      ...
      <section className="px-4">
        <div className="max-w-[900px] mx-auto rounded-[22px] p-6" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)' }}>
          ...
          <div className="flex items-center gap-3 mt-3 text-xs flex-wrap" style={{ color: 'var(--text-tertiary)' }}>
            <span className="px-2.5 py-1 rounded-md" style={{ background: 'var(--bg-pill)' }}>
              Nano Banana 2
            </span>
            <span className="px-2.5 py-1 rounded-md" style={{ background: 'var(--bg-pill)' }}>
              1:1
            ...
            <span className="ml-auto bg-primary text-white px-3 py-1 rounded-md font-medium inline-flex items-center gap-1">
              <Zap className="h-3 w-3" /> Генерировать 300
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4">
        <div className="max-w-[1100px] mx-auto">
          <h2 className="text-[28px] font-bold text-center mb-10" style={{ color: 'var(--text-primary)' }}>
            Возможности Nano Banana 2
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map((f) => (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-[rgba(232, 84, 32,0.15)] flex items-center justify-center mb-3">
      ...
      <section className="max-w-[1000px] mx-auto px-4" style={{ padding: '80px 0' }}>
        <h2 className="text-[28px] font-bold text-center" style={{ color: 'var(--text-primary)', marginBottom: 12 }}>
          Идеальное соответствие персонажей
        </h2>
        ...
        <div className="flex items-center justify-center gap-6 flex-wrap">
          <div>
            <p className="uppercase mb-3" style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-tertiary)', letterSpacing: 1 }}>
            ...
            <div className="flex flex-col gap-2">
              {[aiPhotos[0], aiPhotos[1]].map((src, i) => (
                <div
                  ...
                  style={{ width: 140, height: 180, borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)' }}
                >
      ...
      <section className="py-20 px-4">
        <div className="max-w-[600px] mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)' }}>
            <div
              className="grid grid-cols-2 px-6 py-3 text-xs font-semibold"
              style={{ color: 'var(--text-tertiary)', borderBottom: '1px solid var(--border-primary)' }}
            >
              <span>Модель</span>
              <span className="text-right">Цена за 1 генерацию</span>
            </div>
            {pricing.map((p, i) => (
              <div
                key={p.name}
                className="grid grid-cols-2 px-6 py-4 text-sm"
                style={{ background: i % 2 === 1 ? 'var(--bg-table-row-alt)' : 'transparent' }}
              >
          ...
          <p className="text-center text-[13px] text-primary mt-4">
            Кредиты входят в подписку ERA2.{' '}
      ...
      <section className="py-20 px-4">
        <div className="max-w-[1100px] mx-auto">
          <h2 className="text-[28px] font-bold text-center mb-10" style={{ color: 'var(--text-primary)' }}>
            Почему использовать Nano Banana через ERA2?
          </h2>
            ...
            {whyEra2.map((w) => (
              ...
              >
                <div
                  ...
                  style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(232, 84, 32, 0.15)' }}
                >
                  <w.Icon size={22} style={{ color: 'hsl(var(--primary))' }} />
                </div>
                <h3 className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>
                  {w.title}
                </h3>
          ...
          {[
            ...
            >
              ...
              <div
                ...
                style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', borderRadius: 10, padding: '10px 14px' }}
              >
      ...
      <section className="max-w-[1200px] mx-auto px-4" style={{ padding: '80px 0' }}>
        <h2 className="text-center" style={{ fontSize: 28, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 32 }}>
          Создано с Nano Banana 2
        </h2>
      ...
      <section className="py-20 px-4">
        <div className="max-w-[900px] mx-auto">
          <h2 className="text-[28px] font-bold text-center mb-12" style={{ color: 'var(--text-primary)' }}>
            Как использовать Nano Banana 2 на ERA2
          </h2>
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {otherModels.map((m) => (
              <div
                key={m.name}
                className="flex-shrink-0 w-[180px] rounded-2xl p-4 transition cursor-pointer"
                style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)', boxShadow: 'var(--shadow-card)' }}
              >
                <div className="mb-2">
      ...
      <section className="py-20 px-4">
        <div className="max-w-[700px] mx-auto text-center p-10 rounded-[22px] bg-gradient-to-br from-[rgba(232, 84, 32,0.1)] to-[rgba(255, 122, 61,0.06)]">
          <h2 className="text-[28px] font-bold" style={{ color: 'var(--text-primary)' }}>
            Попробуйте Nano Banana 2 на ERA2
          </h2>
