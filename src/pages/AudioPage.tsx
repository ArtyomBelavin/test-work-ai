...
const welcomeScenarios: WelcomeScenario[] = [
  {
    ...
    prompt:
      'Добрый день! Представляем вам новый продукт, который изменит ваш подход к работе. Это инновационное решение создано для тех, кто ценит качество и эффективность.',
  ...
  },
  {
    ...
    prompt:
      'Сегодня мы поговорим о том, как искусственный интеллект меняет нашу повседневную жизнь. От рекомендаций в музыке до медицинских диагнозов — ИИ проникает во все сферы.',
    providerId: 'elevenlabs',
  },
  {
    Icon: Volume2,
...
const AudioPage = () => {
  ...
  useEffect(() => {
    document.title = 'ERA2 — Генерация аудио'
  }, [])
  useEffect(() => {
    ...
    <ErrorBoundary>
      <div className="flex flex-col h-[calc(100vh-var(--header-height,64px))] mesh-background">
        ...
        <div className="flex-1 overflow-y-auto w-full">
          <div
            className="sticky top-0 z-20 flex justify-center py-2"
            style={{ background: 'color-mix(in oklab, var(--c-bg) 85%, transparent)', backdropFilter: 'blur(12px)' }}
          >
            <div className="relative" data-capsule-dropdown>
              <button
                ...
                style={{ background: 'var(--c-bg-1)', border: '1px solid var(--c-line)', color: 'var(--c-fg)' }}
              >
                <ModelGlyph name={currentModelName} size={20} />
                <span>{currentModelName}</span>
                <span className="text-muted-foreground">·</span>
                <span className="font-mono tabular-nums text-xs" style={{ color: 'var(--c-accent-2)' }}>
                  {currentSubName}
                </span>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div
                            ...
                            style={{ color: selectedModel === m.id ? 'hsl(var(--primary))' : 'hsl(var(--foreground))' }}
                          >
                            {m.name}
                          </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    {voiceTabs.map((t) => (
                      <button
                        ...
                        className={cn('px-3.5 py-1.5 rounded-full text-[13px] font-medium transition-colors', voiceTab === t.id ? 'text-white' : '')}
                        style={
                          voiceTab === t.id
                            ? { background: 'linear-gradient(135deg, hsl(var(--primary)), #ff7a3d)' }
                            : { background: 'var(--bg-pill)', border: '1px solid var(--border-primary)', color: 'var(--text-secondary)' }
                        }
                      >
                        {t.label}
                      </button>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  {[
                    { title: 'МОДЕЛЬ', value: elModel, setter: setElModel, options: ['Speech (Eleven v2)', 'Dialogue/Speech (Eleven v3)'] },
                    {
                      ...
                      value: elLang,
                      setter: setElLang,
                      options: ['Русский', 'Английский', 'Немецкий', 'Французский', 'Испанский', 'Китайский', 'Японский'],
                    },
                      ...
                      {elevenScenarios.map((s) => (
                        ...
                        >
                          <div
                            ...
                            style={{ background: 'rgba(232, 84, 32, 0.1)', border: '1px solid rgba(232, 84, 32, 0.18)' }}
                          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-3 p-4 rounded-xl border transition-colors text-left',
                      selectedModel === m.id ? 'border-[hsl(var(--primary))] bg-[rgba(232, 84, 32,0.05)]' : '',
                    )}
        ...
        <div ref={inputAreaRef} className="shrink-0 px-4 lg:px-6 pb-4 pt-1.5 bg-[var(--bg-primary)] relative z-[1]">
          <div className="max-w-[780px] mx-auto">
            ...
            <div className={isGenerating ? 'glow-border-active' : 'glow-border-idle'}>
              <div className="rounded-[22px] p-5 border border-[hsl(var(--border))] bg-[hsl(var(--card))] transition-all duration-200 has-[textarea:focus]:border-[hsl(var(--primary))] has-[textarea:focus]:shadow-[0_0_0_3px_rgba(232,84,32,0.12),0_1px_4px_rgba(0,0,0,0.2)]">
                <textarea
                  value={prompt}
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  <div className="relative">
                    ...
                    {modelDDOpen && (
                      <div
                        className="absolute bottom-full left-0 mb-1 w-64 rounded-xl shadow-xl z-50 p-1"
                        style={{ background: 'var(--bg-popup)', border: '1px solid var(--border-primary)' }}
                      >
                        {[
                          { id: 'elevenlabs' as const, name: 'ElevenLabs', desc: 'Озвучка и голос', credits: 60 },
                          { id: 'suno' as const, name: 'Suno', desc: 'Генерация музыки', credits: 30 },
                          ...
                          >
                            ...
                            <div className="flex-1 text-left">
                              <div className="font-medium" style={{ color: selectedModel === m.id ? 'hsl(var(--primary))' : 'var(--text-primary)' }}>
                                {m.name}
                              </div>
                  ...
                  {isEL ? (
                    <>
                      <PillDropdown
                        ...
                        value={`Голос: ${selectedVoice}`}
                        options={voices.map((v) => ({ value: v.name, label: v.name }))}
                        onSelect={setSelectedVoice}
                      />
                      <PillDropdown
                        value="v2 Multilingual"
                        options={[
                          { value: 'v2 Multilingual', label: 'v2 Multilingual' },
                          { value: 'v3', label: 'v3' },
                        ]}
                        onSelect={() => {}}
                      />
                  ...
                  >
                    ...
                    <span className="inline-flex items-center gap-1 ml-1 font-mono tabular-nums">
                      <Zap className="w-3 h-3" /> {isEL ? 60 : 30}
                    </span>
                  </motion.button>
                </div>
              </div>
