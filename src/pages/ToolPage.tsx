    ...
    <div className="min-w-0">
      ...
      <section className="relative overflow-hidden" style={{ background: 'linear-gradient(to bottom, hsl(var(--background)), hsl(var(--card)))' }}>
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 80% 100% at 50% 0%, rgba(232,84,32,0.15) 0%, rgba(255,122,61,0.05) 40%, transparent 70%)' }}
        />
        <div className="relative max-w-4xl mx-auto px-4 pt-16 pb-14 md:pt-20 md:pb-16">
          <nav className="flex items-center gap-1.5 text-[13px] text-muted-foreground mb-8">
            ...
            <Link to="/toolkit" className="hover:text-foreground transition-colors">
              Инструменты
            </Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <div className="bg-card border border-border rounded-2xl p-5 md:p-6 max-w-2xl">
            <div className="flex gap-2 mb-4">
              ...
              <span className="px-4 py-1.5 rounded-full text-sm font-medium text-muted-foreground hover:text-foreground cursor-pointer transition-colors border border-border">
                Изображение в изображение
              </span>
            </div>
            <input
              readOnly
              placeholder="Введите текст запроса для генерации"
              className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm placeholder:text-muted-foreground mb-4 outline-none"
            />
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link
                to={targetPage}
                className="ml-auto px-5 py-2 rounded-xl gradient-accent text-white text-sm font-semibold hover:opacity-90 transition-opacity"
              >
                Создать
              </Link>
            </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed">
          Модели {providerTabs[activeProvider]} охватывают различные варианты использования: от фотореалистичной генерации до редактирования на
          естественном языке.
        </p>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="columns-2 md:columns-3 lg:columns-4 gap-3 space-y-3">
          {communityWorks.map((w) => (
            <div
              key={w.author}
              className="break-inside-avoid rounded-xl border border-border relative overflow-hidden cursor-pointer hover:scale-[1.02] transition-transform group"
              style={{ height: `${180 + Math.random() * 80}px`, background: w.gradient }}
            >
            ...
            <div className="space-y-6">
              {[
                { num: '1', title: 'Введите текстовую подсказку', desc: 'Опишите изображение которое хотите создать максимально детально.' },
                {
                  ...
                  desc: 'Создайте своё изображение с помощью нашей ИИ и параметров искусственного интеллекта.',
                },
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/toolkit" className="text-sm text-primary hover:underline">
            Создавайте больше →
          </Link>
        </div>
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
          {imageTools.map((t) => (
            <div
              key={t.label}
              className="shrink-0 w-[180px] h-[120px] border border-border rounded-xl flex flex-col items-center justify-center gap-3 hover:border-primary/30 hover:bg-accent/50 transition-colors cursor-pointer"
            >
              <div
      ...
      <section className="max-w-3xl mx-auto px-4 py-16 md:py-20 text-center">
        ...
        <div
          className="mt-8 max-w-[400px] mx-auto aspect-[4/3] rounded-xl border border-border"
          style={{ background: 'linear-gradient(135deg, #1a1030, #0f1a2e)' }}
        />
      </section>
