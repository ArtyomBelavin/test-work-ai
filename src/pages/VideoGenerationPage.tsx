...
const VideoGenerationPage = () => {
  ...
  useEffect(() => {
    setPageMeta('Генерация видео — ИИ нейросети | ERA2.ai', 'Kling, Sora, Seedance, Veo. Генерация видео из текста и изображений.')
  }, [])
    ...
    <div className="min-w-0" style={{ background: 'var(--seo-bg)' }}>
      ...
      <section className="relative overflow-hidden" style={{ paddingTop: 80, paddingBottom: 80 }}>
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at 50% 0%, rgba(232, 84, 32,0.18) 0%, rgba(255, 122, 61,0.08) 40%, transparent 70%)',
          }}
        />
      ...
      <section style={{ padding: '80px 0' }}>
        <div className="max-w-[1200px] mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold mb-8" style={{ color: 'var(--seo-heading)' }}>
            Доступные нейросети
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {models.map((m) => (
              ...
              >
                ...
                <span className="font-mono tabular-nums" style={{ fontSize: 13, color: 'hsl(var(--primary))' }}>
                  от {m.credits} cr
                </span>
              </div>
            ))}
              ...
              <thead>
                <tr style={{ background: 'var(--seo-table-header)' }}>
                  {['Модель', 'Цена, cr', 'Макс. длина', 'Разрешение', 'Качество'].map((h) => (
                    <th key={h} className="text-left font-semibold" style={{ padding: '12px 16px', color: 'var(--seo-table-text)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
      ...
      <section style={{ padding: '80px 0' }}>
        <div className="max-w-[1200px] mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold mb-8 text-center" style={{ color: 'var(--seo-heading)' }}>
          ...
          {faqData.map((item, i) => (
            <div key={i} style={{ borderBottom: '1px solid var(--seo-faq-border)', padding: '20px 0' }}>
              <button className="w-full flex items-center justify-between text-left" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                ...
                {openFaq === i ? (
                  <Minus className="shrink-0 ml-4" style={{ width: 18, height: 18, color: 'var(--seo-text-muted)' }} />
                ) : (
                  <Plus className="shrink-0 ml-4" style={{ width: 18, height: 18, color: 'var(--seo-text-muted)' }} />
                )}
              </button>
              {openFaq === i && (
      ...
      <section style={{ padding: '80px 0' }}>
        <div className="max-w-[1200px] mx-auto px-4">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-bold mb-4" style={{ color: 'var(--seo-heading)' }}>
