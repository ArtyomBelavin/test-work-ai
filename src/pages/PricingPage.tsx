    ...
    <div className="min-w-0">
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 80% 100% at 50% 0%, rgba(232,84,32,0.12) 0%, rgba(255,122,61,0.04) 40%, transparent 70%)' }}
        />
        <motion.div className="relative max-w-4xl mx-auto text-center px-4 pt-16 pb-10 md:pt-20" initial="hidden" animate="show" variants={stagger}>
          <motion.p variants={fadeUp} className="text-xs uppercase tracking-[2px] text-primary font-semibold mb-4">
            Тарифы
          </motion.p>
          <motion.h1 variants={fadeUp} className="text-[32px] md:text-[44px] font-bold mb-4">
            Простые тарифы для каждого
          </motion.h1>
          <motion.p variants={fadeUp} className="text-muted-foreground mb-8">
            Одна подписка — доступ ко всем 90+ нейросетям. Начните бесплатно.
          </motion.p>
          ...
          <motion.div variants={fadeUp} className="inline-flex rounded-full border border-[hsl(var(--border))] p-1">
            <button
              ...
              className={cn(
                ...
                period === 'year' ? 'gradient-accent text-white' : 'text-muted-foreground hover:text-foreground',
              )}
            >
              Год
            ...
            <button
              ...
              className={cn(
                ...
                period === 'month' ? 'gradient-accent text-white' : 'text-muted-foreground hover:text-foreground',
              )}
            >
              Месяц
            </button>
            ...
            return (
              ...
              >
                ...
                <div className="mb-2 min-h-[20px]">
                  {plan.oldPrice && <span className="font-mono tabular-nums text-sm text-muted-foreground line-through mr-2">{plan.oldPrice}</span>}
                </div>
                <div className="mb-4">
                ...
                <button
                  className={cn(
                    ...
                    isPrimaryCta ? 'gradient-accent text-white hover:opacity-90' : 'border border-border text-foreground hover:bg-muted',
                  )}
                >
                  {plan.cta}
                </button>
          ...
          {faqItems.map((item, i) => (
            <div key={i}>
              ...
              >
                <span>{item.q}</span>
                <ChevronDown
                  className={cn('h-4 w-4 shrink-0 ml-4 text-muted-foreground transition-transform duration-200', openFaq === i && 'rotate-180')}
                />
              </button>
              <div className={cn('overflow-hidden transition-all duration-200', openFaq === i ? 'max-h-40 pb-4' : 'max-h-0')}>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-16">
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 80% 100% at 50% 0%, rgba(232,84,32,0.18) 0%, rgba(255,122,61,0.06) 40%, transparent 75%), hsl(var(--card))',
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-bold mb-3 text-foreground">Приведи друга — получи 100 кредитов</h2>
          <p className="text-sm md:text-base text-muted-foreground max-w-xl mx-auto mb-6 leading-relaxed">
            За каждого друга, который зарегистрируется по вашей ссылке, вы оба получите по 100 бесплатных кредитов. Без ограничений на количество
            приглашений.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-5">
            <div
              className="px-4 py-2.5 rounded-full font-mono text-sm text-foreground"
          ...
          <p className="text-xs text-muted-foreground font-mono">Уже пригласили: 0 друзей · Заработано: 0 кредитов</p>
        </div>
      </section>
