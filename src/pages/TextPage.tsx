...
const TextPage = () => {
  ...
  useEffect(() => {
    document.title = 'ERA2 — Текстовые нейросети'
  }, [])
  useEffect(() => {
  ...
  return (
    <ErrorBoundary>
      <div className="flex flex-col h-[calc(100vh-var(--header-height,64px))] mesh-background" style={{ background: c.bg }}>
        ...
        <div className="flex-1 overflow-y-auto relative z-[1]">
          ...
          >
            <div className="relative">
              ...
              >
                ...
                <span className="font-mono text-xs" style={{ color: 'var(--c-accent-2)' }}>
                  {subModel.name}
                </span>
                <span className="font-mono text-xs" style={{ color: 'var(--c-accent-2)' }}>
                  {subModel.credits} cr
          ...
          {!hasMessages && (
            <div className="px-3 sm:px-4 lg:px-8 py-6 space-y-6 border-t mt-8 sm:mt-16" style={{ borderColor: c.divider }}>
              ...
              <div>
                <h2 className="text-base font-bold mb-4" style={{ color: c.textPrimary }}>
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-2 pt-2 overflow-x-auto no-scrollbar" style={{ borderTop: `1px solid ${c.divider}` }}>
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.background = c.pillBg
                  ...
                  >
                    ...
                    <span className="font-mono" style={{ color: c.textAccent, background: c.badgeBg, padding: '1px 6px', borderRadius: '999px' }}>
                      {subModel.credits} cr
                    </span>
                  ...
                  {subModel.hasWeb && (
                    <button
                      ...
                      className={cn('flex items-center gap-1 px-3 py-1.5 rounded-full text-xs border transition-colors', webSearch ? '' : '')}
                      style={
                        webSearch
                  ...
                  {subModel.hasThinking && (
                    <button
                      ...
                      className={cn('flex items-center gap-1 px-3 py-1.5 rounded-full text-xs border transition-colors')}
                      style={
                        thinking
                          ? { borderColor: c.textAccent, background: `${c.textAccent}1a`, color: c.textAccent }
                          : { background: c.pillBg, borderColor: c.pillBorder, color: c.textSecondary }
        ...
        {systemDrawerOpen && <SystemPromptDrawer value={systemPrompt} onChange={setSystemPrompt} onClose={() => setSystemDrawerOpen(false)} />}
      </div>
    </ErrorBoundary>
  )
}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-4 py-4">
      <div className="flex flex-col items-center text-center max-w-md">
        ...
        <h1 className="text-[22px] font-semibold mb-0.5 tracking-tight" style={{ color: c.textPrimary }}>
          {providerName}
        </h1>
        <p className="text-[13px] mb-0.5 font-mono tabular-nums" style={{ color: c.textSecondary }}>
          {subModelName}
        </p>
        <p className="text-[13px] mb-5" style={{ color: c.textTertiary }}>
          Единый доступ к 90+ нейросетям
        </p>
    ...
    <div className="max-w-[780px] mx-auto py-6 px-4 space-y-5">
      {messages.map((msg) => (
        <div key={msg.id} className={cn('flex gap-3', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
          {msg.role === 'assistant' && <ModelIcon providerId={msg.providerId || currentProviderId} size={24} className="mt-1" />}
          <div className="max-w-[75%]">
            {msg.role === 'assistant' && (
              <div className="text-[11px] mb-1 font-medium" style={{ color: c.textMuted }}>
                {msg.model || currentModel}
              </div>
            )}
            <div
              className={cn('rounded-[14px] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap', msg.role === 'user' ? 'rounded-br-[4px]' : '')}
              style={msg.role === 'user' ? { background: c.userBubble, color: '#FFFFFF' } : { background: c.assistantBubble, color: c.assistantText }}
            >
              {msg.content}
            </div>
            {msg.role === 'assistant' && (
            ...
            <div className="flex items-center gap-1.5 px-2 py-3">
              {[0, 150, 300].map((delay) => (
                <span
                  ...
                  style={{ background: c.textAccent, animationDelay: `${delay}ms` }}
                />
              ))}
            </div>
          </div>
    ...
    >
      <div className="max-w-[780px] mx-auto space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-sm" style={{ color: c.textSecondary }}>
            ✕
          </button>
        </div>
        <textarea
