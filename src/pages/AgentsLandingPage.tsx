...
const agents: { Icon: LucideIcon; name: string; description: string }[] = [
  {
    Icon: PenLine,
    name: 'Копирайтер',
    description: 'Пишет продающие тексты, посты для соцсетей, email-рассылки и рекламные заголовки.',
  },
  {
    Icon: Code,
    name: 'Программист',
    description: 'Пишет код, находит ошибки, объясняет алгоритмы. Python, JavaScript, SQL и другие языки.',
  },
  {
    Icon: Palette,
    name: 'Дизайн-ассистент',
    description: 'Помогает составить промпт для генерации изображений, подбирает стили и композицию.',
  },
  {
    Icon: BarChart3,
  ...
  },
  {
    Icon: GraduationCap,
    name: 'Репетитор',
    description: 'Объясняет сложные темы простым языком, помогает учиться и готовиться к экзаменам.',
  },
]
...
export default function AgentsLandingPage() {
  ...
  useEffect(() => {
    setPageMeta('ИИ-агенты и ассистенты | ERA2.ai', '44 ИИ-ассистента для образования, бизнеса, маркетинга, разработки, здоровья.')
  }, [])
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--seo-bg)' }}>
      <section
        ...
        style={{
          background: 'radial-gradient(ellipse 60% 40% at 50% 0%, rgba(232, 84, 32,0.2), rgba(255, 122, 61,0.1), transparent 70%)',
        }}
      >
        <div className="max-w-[1200px] mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-6" style={{ color: 'var(--seo-heading)' }}>
            ИИ Агенты и ассистенты
          </h1>
          <p className="text-lg mb-8 max-w-2xl mx-auto" style={{ color: 'var(--seo-text)' }}>
            Готовые ИИ-ассистенты для бизнеса и творчества. Настроенные промпты, специализированные роли — начните работу за секунду.
          </p>
          <Link
            to={cta}
      ...
      <section style={{ padding: '80px 0' }} className="px-4">
        <div className="max-w-[1200px] mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12" style={{ color: 'var(--seo-heading)' }}>
            Доступные агенты
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {agents.map((a) => (
              ...
              >
                ...
                <Link to={cta} className="mt-4 text-[13px] font-medium hover:underline" style={{ color: 'hsl(var(--primary))' }}>
                  Попробовать →
                </Link>
              </div>
            ))}
