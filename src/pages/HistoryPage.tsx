        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-24">
            ...
            <p className="text-sm text-muted-foreground">
              {filter === 'favorites' ? 'Вы ещё не отметили ничего избранным' : 'Попробуйте сменить фильтр'}
            </p>
          </div>
        ) : (
          <div className="columns-2 md:columns-3 lg:columns-4 gap-3 mt-4 pb-12">
            {filtered.map((item) => (
              <HistoryCard key={item.id} item={item} onClick={() => setSelected(item)} onToggleFavorite={toggleFavorite} />
            ))}
          </div>
        )}
      </div>

      <HistoryDetailDialog item={selected} open={selected !== null} onOpenChange={(o) => !o && setSelected(null)} />
    </div>
  )
}
