...
export function ModelSelectorDropdown({ open, onClose, selectedModelId, selectedSubModelId, onSelect }: Props) {
  ...
  const filtered = textModels.filter(
    (m) => m.name.toLowerCase().includes(q) || m.provider.toLowerCase().includes(q) || m.subModels?.some((s) => s.name.toLowerCase().includes(q)),
  )
...
function ProviderList({
  ...
  return (
    <div className="py-1">
      ...
      {models.length === 0 && <div className="px-4 py-8 text-center text-sm text-muted-foreground">Ничего не найдено</div>}
    </div>
  )
}
      ...
      {model.subModels?.map((sub) => {
        ...
        return (
          <button
            ...
            className={cn('w-full flex items-center gap-3 px-4 py-2.5 hover:bg-muted/60 transition-colors text-left', isSelected && 'bg-muted/40')}
          >
            <div className="flex-1 min-w-0">
              <span className="text-sm">{sub.name}</span>
              {sub.isNew && <span className="ml-2 text-[10px] font-mono font-bold bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">NEW</span>}
            </div>
            <span className="font-mono text-xs bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full shrink-0">
              {sub.credits} cr
            </span>
