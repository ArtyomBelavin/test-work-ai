...
export function TextModelSelector({ open, onClose, selectedProviderId, selectedSubModelId, onSelect, anchorRef }: Props) {
  ...
  const selector = (
    <>
      ...
      <div
        ref={ref}
        className={cn('flex gap-1', position ? 'fixed z-[100]' : 'absolute top-full left-1/2 -translate-x-1/2 mt-2 z-[100]')}
        style={position ? { top: position.top, left: position.left, transform: 'translateX(-50%)' } : undefined}
      >
        {/* Main dropdown */}
        <div
          className="w-[340px] max-h-[480px] rounded-xl border shadow-2xl overflow-hidden flex flex-col"
          style={{ background: 'var(--bg-popup)', borderColor: 'rgba(232, 84, 32,0.3)' }}
        >
          ...
          <div className="flex-1 overflow-y-auto">
            {isSearching ? (
              <>
                  ...
                  provider.subModels.map((sub) => {
                    ...
                    return (
                      ...
                      >
                            ...
                            {sub.badge && (
                              <span
                                className={cn(
                                  ...
                                  badgeStyles[sub.badge] || 'bg-muted text-muted-foreground',
                                )}
                              >
                                {sub.badge}
                              </span>
                        ...
                        <span className="font-mono text-xs shrink-0" style={{ color: 'hsl(var(--primary))' }}>
                          {sub.credits} cr
                        </span>
                ...
                {filteredProviders.length === 0 && <div className="px-4 py-8 text-center text-sm text-muted-foreground">Модель не найдена</div>}
              </>
            ) : (
        ...
        {!isSearching && hoverProviderData && (
          <div
            className="w-[300px] max-h-[480px] rounded-xl border shadow-2xl overflow-y-auto"
            style={{ background: 'var(--bg-popup)', borderColor: 'rgba(232, 84, 32,0.3)' }}
          >
            ...
            {hoverProviderData.subModels.map((sub) => {
              ...
              return (
                <button
                  key={sub.id}
                  onClick={() => {
                    onSelect(hoverProviderData.id, sub.id)
                    onClose()
                  }}
                  className={cn(
