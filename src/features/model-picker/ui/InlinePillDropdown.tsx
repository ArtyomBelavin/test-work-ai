...
export function InlinePillDropdown({ icon, value, options, onSelect, className }: InlinePillDropdownProps) {
  ...
  return (
    <div ref={ref} className="relative inline-flex">
      ...
      >
        {icon && <span className="inline-flex items-center justify-center w-3.5 h-3.5 opacity-70 [&_svg]:w-3.5 [&_svg]:h-3.5">{icon}</span>}
        <span className="font-mono tabular-nums leading-none">{current?.label || value}</span>
        <ChevronDown size={12} className={cn('opacity-60 transition-transform duration-150', open ? 'rotate-180' : '')} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
          ...
          >
            {options.map((opt) => (
              ...
              >
                ...
                {opt.desc && <span className="text-[11px] text-[hsl(var(--muted-foreground))] font-normal">{opt.desc}</span>}
              </button>
            ))}
          </motion.div>
        )}
