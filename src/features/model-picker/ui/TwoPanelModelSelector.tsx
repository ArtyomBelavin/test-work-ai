...
function getBadgeStyle(badge: string): { bg: string; color: string } {
  ...
  if (lower.includes('топ') || lower.includes('top')) return { bg: 'rgba(255, 122, 61,0.2)', color: '#ff7a3d' }
  if (lower.includes('off') || lower.includes('скидк')) return { bg: 'rgba(52,211,153,0.2)', color: '#34d399' }
  if (lower.includes('new') || lower.includes('нов')) return { bg: 'rgba(232, 84, 32,0.2)', color: 'hsl(var(--primary))' }
  if (lower.includes('google')) return { bg: 'rgba(96,165,250,0.2)', color: '#60a5fa' }
  if (lower.includes('openai')) return { bg: 'rgba(255,255,255,0.1)', color: 'var(--text-secondary)' }
  if (lower.includes('open source')) return { bg: 'rgba(74,222,128,0.2)', color: '#4ade80' }
  if (lower.includes('premium')) return { bg: 'rgba(232, 84, 32,0.2)', color: 'hsl(var(--primary))' }
  if (lower.includes('sota')) return { bg: 'rgba(96,165,250,0.2)', color: '#60a5fa' }
  if (lower.includes('creative')) return { bg: 'rgba(255, 122, 61,0.2)', color: '#ff7a3d' }
  if (lower.includes('xai')) return { bg: 'rgba(96,165,250,0.2)', color: '#60a5fa' }
...
export function TwoPanelModelSelector({ providers, selectedProviderId, selectedSubModelId, onSelect, triggerButtonRef }: TwoPanelModelSelectorProps) {
      ...
      {open && (
        ...
        >
          ...
          <div className="flex flex-1 min-h-0">
            {/* Left: providers */}
            <div
              className="flex-shrink-0 overflow-y-auto w-[180px] sm:w-[220px]"
              style={{ borderRight: '1px solid var(--border-primary)', paddingTop: 8 }}
            >
              {providers.map((p) => {
                    ...
                    {p.badge &&
                      (() => {
                        ...
                        return (
                          <span
                            className="shrink-0"
                            style={{ fontSize: 9, padding: '1px 4px', borderRadius: 4, background: s.bg, color: s.color, fontWeight: 600 }}
                          >
                            {p.badge}
                          </span>
                        )
            ...
            <div className="flex-1 submodel-scroll-container" style={{ minHeight: 0, paddingTop: 8, overflowY: 'scroll', scrollbarGutter: 'stable' }}>
              {hoverProvider?.subModels.map((s, i) => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => {
                      onSelect(hoverProvider.id, s.id)
                      setOpen(false)
                    }}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div
                        ...
                        className="font-mono tabular-nums inline-flex items-center gap-2"
                      >
                        <span className="inline-flex items-center gap-1">
                          <Clock size={10} /> {s.time || '20 сек'}
                        </span>
                        <span style={{ opacity: 0.5 }}>·</span>
                        <span className="inline-flex items-center gap-1" style={{ color: 'hsl(var(--primary))' }}>
                          <Zap size={10} /> {s.credits} cr
                        </span>
                      </div>
                    </div>
