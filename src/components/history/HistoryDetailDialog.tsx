      ...
      {bars.map((h, i) => (
        <div
          ...
          style={{
            height: `${h}%`,
            background:
              i < 22 ? 'linear-gradient(to top, hsl(var(--primary)), #ff7a3d)' : 'color-mix(in oklab, hsl(var(--muted-foreground)) 50%, transparent)',
          }}
        />
      ))}
    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[60%_40%]">
          ...
          <div className="bg-[hsl(var(--background))] flex items-center justify-center min-h-[280px] md:min-h-[480px] p-4 md:p-6">
            ...
            {type === 'audio' && (
              <div className="w-full rounded-xl bg-secondary p-6 flex flex-col items-center gap-4">
                ...
                <BigWaveform />
                {duration && <span className="font-mono tabular-nums text-sm text-muted-foreground">{duration}</span>}
              </div>
            )}
          ...
          <div className="flex flex-col p-6 gap-4 border-t md:border-t-0 md:border-l border-border bg-card">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {aspect && <span className="font-mono text-[11px] px-2 py-0.5 rounded-full bg-secondary text-muted-foreground">{aspect}</span>}
              {duration && <span className="font-mono text-[11px] px-2 py-0.5 rounded-full bg-secondary text-muted-foreground">{duration}</span>}
            </div>

            <div>
              <div className="font-mono uppercase tracking-wide text-[11px] text-muted-foreground mb-1.5">Промпт</div>
            ...
            <div className="flex flex-col gap-2">
              <Button onClick={handleReuse} className="w-full gradient-accent text-white shadow-[0_10px_30px_-10px_rgba(232,84,32,0.55)]">
                <Sparkles size={16} />
                Использовать снова
              </Button>
