      ...
      {bars.map((h, i) => (
        <div
          ...
          style={{
            height: `${h}%`,
            background:
              i < 10 ? 'linear-gradient(to top, hsl(var(--primary)), #ff7a3d)' : 'color-mix(in oklab, hsl(var(--muted-foreground)) 45%, transparent)',
          }}
        />
      ))}
    </div>
    ...
    >
      ...
      <div className="relative">
        {(type === 'image' || type === 'video') && (
          <div className={`${ASPECT_CLASS[aspect ?? '1:1']} w-full`} style={{ background: gradient }}>
            {type === 'video' && (
        ...
        {type === 'text' && (
          <div className="relative bg-secondary px-3.5 pt-3.5 pb-6 min-h-[120px]">
            <p className="text-sm text-foreground/90 whitespace-pre-line line-clamp-4 leading-relaxed">{text}</p>
            <div
              className="pointer-events-none absolute inset-x-0 bottom-0 h-10"
              style={{
                background: 'linear-gradient(to bottom, transparent, hsl(var(--card)) 100%)',
              }}
            />
          </div>
        )}
        ...
        <button
          ...
          className={`absolute top-2 right-2 w-7 h-7 rounded-full backdrop-blur-sm flex items-center justify-center transition-all ${
            favorite ? 'opacity-100 text-white' : 'opacity-0 group-hover:opacity-100 text-white/90 bg-black/40 hover:bg-black/60'
          }`}
          style={favorite ? { background: 'linear-gradient(135deg, hsl(var(--primary)), #ff7a3d)' } : undefined}
        >
