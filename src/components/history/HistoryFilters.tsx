...
export function HistoryFilters({ value, onChange, counts }: Props) {
  return (
    <div className="sticky z-30 bg-background/85 backdrop-blur-md border-b border-border" style={{ top: 'var(--header-height, 56px)' }}>
        ...
        {OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              {opt.label}
              {typeof count === 'number' && (
                <span className={active ? 'text-white/80 font-mono text-[11px]' : 'text-muted-foreground/70 font-mono text-[11px]'}>{count}</span>
              )}
            </button>
          )
        })}
