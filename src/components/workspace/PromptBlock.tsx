...
function SegmentedDropdown({
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (ref.current && !ref.current.contains(e.target as Node) && anchor.current && !anchor.current.contains(e.target as Node)) onClose()
    }
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full mt-2 z-50 min-w-[200px] max-w-[320px] rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--popover))] p-1.5 shadow-2xl max-h-[400px] overflow-y-auto"
    >
      {children}
    </div>
  )
                  ...
                  >
                    <div className="flex flex-col items-start min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {m.isNew && (
                          <span className="font-mono text-[9px] px-1 py-px rounded bg-[hsl(var(--primary))]/15 text-[hsl(var(--primary))] border border-[hsl(var(--primary))]/30 uppercase tracking-wider">
                            NEW
                          </span>
                        )}
                      </div>
