        ...
        >
          {options.map((o) => (
            <button
              ...
              className={cn('w-full text-left px-3 py-2 rounded-[8px] text-[13px] transition-colors', mono && 'font-mono tabular-nums')}
              style={{
    ...
    return (
      <>
        ...
        <ParamPill
          ...
          mono
        />
        <ParamPill Icon={Hash} label="Варианты" value={imgVar} onChange={setImgVar} options={['1 вариант', '2 варианта', '4 варианта']} />
      </>
    )
  }
  if (type === 'video') {
    return (
      <>
        ...
        <ParamPill
          ...
          mono
        />
        <ParamPill Icon={Clock} label="Длительность" value={vidDur} onChange={setVidDur} options={['3s', '5s', '8s', '10s']} mono />
        <ParamPill Icon={Monitor} label="Разрешение" value={vidRes} onChange={setVidRes} options={['480p', '720p', '1080p']} mono />
      </>
    )
  }
  return (
    <>
      ...
      />
      <ParamPill Icon={Clock} label="Длительность" value={audDur} onChange={setAudDur} options={['1 мин', '2 мин', '3 мин', '4 мин']} mono />
      <ParamPill
        Icon={Disc}
        label="Жанр"
        value={audGenre}
        onChange={setAudGenre}
...
export function PromptWindow({ type, onTypeChange, prompt, onPromptChange, onGenerate }: PromptWindowProps) {
  ...
  const cardBorder = active ? 'color-mix(in oklab, var(--c-accent) 40%, transparent)' : 'var(--c-line-2)'
    ...
    <div className="w-full max-w-[780px] mx-auto">
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={
                isActive
                  ? {
                      ...
                      boxShadow: active
                        ? '0 -12px 34px -18px var(--c-accent-glow), -12px 0 28px -22px var(--c-accent-glow), 12px 0 28px -22px var(--c-accent-glow)'
                        : 'none',
                    }
                  : {
                      height: 40,
              ...
              onMouseLeave={(e) => {
                if (!isActive) e.currentTarget.style.color = 'color-mix(in oklab, var(--c-fg-dim) 70%, transparent)'
              }}
            >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: active ? '0 0 0 1px var(--c-accent), 0 0 40px -5px var(--c-accent-glow)' : 'none',
        }}
      >
        {/* Row 1: + icon + textarea */}
        ...
        <div className="flex items-center gap-2 mt-4 min-h-10">
          ...
          <button
            ...
            style={{
              ...
              color: '#fff',
              boxShadow: '0 10px 30px -10px var(--c-accent-glow), inset 0 1px 0 rgba(255,255,255,0.25)',
            }}
            onMouseEnter={(e) => {
