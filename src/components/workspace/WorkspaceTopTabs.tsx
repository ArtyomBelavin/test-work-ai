...
export function WorkspaceTopTabs({ value, onChange }: Props) {
  return (
    ...
    >
      <div className="inline-flex items-center gap-1 p-1 rounded-full" style={{ background: 'var(--c-bg-2)', border: '1px solid var(--c-line)' }}>
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active ? 'text-white shadow-[0_4px_14px_-4px_rgba(232,84,32,0.5)]' : 'text-muted-foreground hover:text-foreground',
              )}
              style={active ? { background: 'linear-gradient(135deg, hsl(var(--primary)), #ff7a3d)' } : undefined}
            >
              {t.label}
            </button>
          )
