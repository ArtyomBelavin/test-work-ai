...
export function ModelsGrid3x3({ models, onSelect }: ModelsGrid3x3Props) {
  return (
    <div className="mt-10">
      <h2 className="text-[18px] font-bold mb-5 tracking-tight" style={{ color: 'var(--text-primary)' }}>
        Модели
      </h2>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {m.badges?.map((b) => {
                  ...
                  return (
                    <StatusBadge key={b} variant={isOff ? 'top' : 'beta'}>
                      {b}
                    </StatusBadge>
                  )
                })}
              </div>
              <p className="text-[13px] whitespace-nowrap overflow-hidden text-ellipsis" style={{ color: 'var(--text-tertiary)' }}>
                {m.desc}
              </p>
            </div>
