...
export function ModelsGrid({ title, models, onSelect }: ModelsGridProps) {
  return (
    <div>
      <h2 className="text-[20px] font-bold mb-4" style={{ color: 'var(--text-primary)' }}>
        {title}
      </h2>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {m.isNew && (
                  <span className="text-[8px] bg-gradient-to-r from-[hsl(var(--primary))] to-[#ff7a3d] text-white px-1 py-0.5 rounded-full font-bold shrink-0">
                    NEW
                  </span>
                )}
              </div>
              <p className="text-[11px] whitespace-nowrap overflow-hidden text-ellipsis" style={{ color: 'var(--text-tertiary)' }}>
                {m.desc}
              </p>
            </div>
            {m.credits !== undefined && (
