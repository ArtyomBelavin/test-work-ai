...
const WAVE_BARS = Array.from({ length: 28 }, (_, i) => Math.round(35 + Math.abs(Math.sin(i * 1.7)) * 60))
...
function AudioResult({ gen }: { gen: MediaGeneration }) {
  return (
    <div className="rounded-xl p-3 flex items-center gap-3" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)' }}>
      <button className="shrink-0 w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'hsl(var(--primary))' }}>
        <Play className="w-4 h-4 text-white ml-0.5" fill="currentColor" />
      </button>
      <div className="flex-1 flex items-center gap-[2px] h-9">
        {WAVE_BARS.map((h, i) => (
          <span key={i} className="w-[3px] rounded-sm" style={{ height: `${h}%`, background: 'hsl(var(--primary) / 0.55)' }} />
        ))}
      </div>
