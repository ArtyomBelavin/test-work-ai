  ...
  return (
    ...
    >
      <motion.div variants={fadeUp} className="text-center mb-8">
        ...
        <p className="text-base text-muted-foreground">Создавайте контент и тренды на базе лучших ИИ</p>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="flex justify-center mb-8">
        <div className="inline-flex gap-1 p-1 rounded-xl bg-muted/50 dark:bg-[rgba(255,255,255,0.06)]">
          <button
            ...
            className={cn(
              'px-5 py-2 rounded-[8px] text-sm cursor-pointer',
              tab === 'image' ? 'text-foreground font-medium' : 'text-muted-foreground',
            )}
          ...
          <button
            ...
            className={cn(
              'px-5 py-2 rounded-[8px] text-sm cursor-pointer',
              tab === 'video' ? 'text-foreground font-medium' : 'text-muted-foreground',
            )}
      ...
      <motion.div variants={fadeUp} className="flex gap-4 overflow-x-auto scrollbar-hide pb-4">
        {tools.map((tool, i) => (
          ...
          >
            <img src={allImages[i % allImages.length]} alt="" loading="lazy" className="h-[140px] w-full object-cover" />
            <div className="p-3" style={{ background: 'var(--seo-card-bg)' }}>
              <p className="text-sm font-semibold" style={{ color: 'var(--seo-heading)' }}>
                {tool.name}
              </p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--seo-text-muted)' }}>
                {tool.sub}
              </p>
            </div>
          </div>
