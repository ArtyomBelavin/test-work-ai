...
export function HomeHowItWorks() {
  return (
    <section className="max-w-[900px] mx-auto" style={{ padding: '80px 16px' }}>
      <h2 className="text-foreground" style={{ fontSize: 28, fontWeight: 700, textAlign: 'center', marginBottom: 48 }}>
        Как это работает
      </h2>
        ...
        {steps.map((s) => (
          <div key={s.num}>
            <span
              ...
              style={{ fontSize: 56, fontWeight: 500, lineHeight: 1, color: 'hsl(var(--primary))', letterSpacing: '-0.02em' }}
            >
              {s.num}
            </span>
            <h3 className="text-foreground" style={{ fontSize: 18, fontWeight: 600, marginTop: 12 }}>
              {s.title}
            </h3>
            <p className="text-muted-foreground" style={{ fontSize: 14, marginTop: 8 }}>
              {s.desc}
            </p>
          </div>
        ))}
