...
const faqItems = [
  {
    ...
    a: 'Да. При регистрации вы получаете бесплатные кредиты для тестирования всех нейросетей. Карта не требуется.',
  },
  {
    q: 'Какие нейросети доступны по подписке?',
    a: 'Более 90 моделей: ChatGPT, Claude, Gemini, MidJourney, Nano Banana, Kling, Seedance, Sora, Veo, ElevenLabs, Suno и другие.',
  },
  {
    q: 'Что такое кредиты?',
    a: 'Кредиты (cr) — это внутренняя валюта ERA2. Каждая генерация стоит определённое количество кредитов. Стоимость зависит от модели.',
  },
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-[60px]">
          ...
          <div className="lg:w-[30%] shrink-0">
            <h2 className="text-[32px] font-extrabold leading-[1.2]" style={{ color: 'var(--text-primary)' }}>
              Важные вопросы
            </h2>
          ...
          <div className="flex-1 space-y-2">
            {faqItems.map((item, i) => (
              ...
              >
                <button onClick={() => setOpenIdx(openIdx === i ? null : i)} className="w-full flex items-center justify-between text-left">
                  <span className="text-[15px] font-medium" style={{ color: 'var(--text-primary)' }}>
                    {item.q}
                  </span>
                  <ChevronDown
                    className={cn('h-4 w-4 shrink-0 ml-4 transition-transform duration-200', openIdx === i && 'rotate-180')}
