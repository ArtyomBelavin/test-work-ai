 ...
 *   </SegmentedToolbar>
 */
export const SegmentedToolbar = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
  ...
  ({ icon, label, active, trailing, className, children, ...props }, ref) => {
    ...
    return (
      <button
        ...
        {...props}
      >
        {icon && <span className="inline-flex items-center justify-center w-3.5 h-3.5 [&_svg]:w-3.5 [&_svg]:h-3.5 shrink-0">{icon}</span>}
        {label != null && <span className="leading-none">{label}</span>}
        {children}
        {trailing !== null && (trailing ?? (showChevron ? <ChevronDown className="w-3 h-3 shrink-0 opacity-60 group-hover:opacity-100" /> : null))}
      </button>
    )
  },
)
