...
export const Link = forwardRef<HTMLAnchorElement, LinkProps>(function Link({ to, search, activeProps, onClick, className, ...props }, ref) {
  const { location, navigate } = useRouterContext()
  const href = createHref({ to, search })
  const isActive = location.pathname === to || (to !== '/' && location.pathname.startsWith(to + '/'))
  const activeClassName = typeof activeProps?.className === 'string' ? activeProps.className : undefined
